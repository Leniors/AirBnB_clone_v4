$( document ).ready(function() {
	...
	$('.filter_amenities input[type="checkbox"]').change(function() {
	...
	$('.locations input[type="checkbox"]').change(function() {
		...
		$('.locations h4:first').text(statesList || 'No States selected.');
	});

	$('.filters button').on('click', function(){
		data = {}
		if (Object.keys(selectedAmenities).length !== 0) {
			data['amenities'] = Object.values(selectedAmenities);
		}
		const options = {
			method: 'POST',
			headers: {
